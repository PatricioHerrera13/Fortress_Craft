...
public class DeteccionDisparo2 : MonoBehaviour
{
    ...
    void Update()
    {
        if (enZonaRecarga && Input.GetKey(KeyCode.I) && !haDisparado && VerificarPrefabEnManos())
        {
            Debug.Log("Player 1 interactúa con el cañón y tiene el prefab requerido en las manos");
            StartCoroutine(MoverCañon(cañon2, dirIzquierda, posInicialCañon, puntoDisparo1, Vector3.right));
        }
    }
