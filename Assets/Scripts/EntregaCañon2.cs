...
public class EntregaCañon2 : MonoBehaviour
{
    public List<ItemRequerido2> itemsRequeridos; // Lista de objetos requeridos
    public DeteccionDisparo2 deteccion; // Referencia al script Torreta
    public EntregaCañon2 detector;
    ...
    private void Start()
    {
        if (deteccion != null)
        {
            deteccion.enabled = false; // Asegúrate de que el script Torreta esté desactivado al inicio
        }
    }
    ...
    private void Update()
    {
        if (canDeliver && Input.GetKeyDown(KeyCode.I)) // Cambia la tecla según lo necesites
        {
            if (currentPlayer != null)
            {
                ...
                if (objetoEnMano != null)
                {
                    ...
                    if (itemComponente != null)
                    {
                        ...
                        if (currentPlayer.GetComponent<PlayerVS1>() != null)
                        {
                            currentPlayer.GetComponent<PlayerVS1>().RemoverPrefabDelHandPoint();
                        }
                        else if (currentPlayer.GetComponent<PlayerVS2>() != null)
                        {
                            currentPlayer.GetComponent<PlayerVS2>().RemoverPrefabDelHandPoint();
                        }
    ...
    private void ActivarTorreta()
    {
        if (deteccion != null)
        {
            deteccion.enabled = true; // Activa el script Torreta
