...
public class FireSpawner2 : MonoBehaviour
{
    ...
    void GenerateFire()
    {
        ...
        if (fireSpawnArea.bounds.Contains(newPosition))
        {
            ...
            else
            {
                ...
                Fuego2[] existingFires = FindObjectsOfType<Fuego2>(); // Obtener todos los fuegos existentes
                if (existingFires.Length > 0)
                {
                    // Elegir un fuego existente al azar
                    Fuego2 randomFire = existingFires[Random.Range(0, existingFires.Length)];
